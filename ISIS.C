...
main()
{
    ...
    /* Initialize ISIS-II file system */
    initisis();
    ...
    /* Check if program was specified on command line */
    ...
    if (count > 0)
    {
        /* Program specified on command line - load and run it once */
        go8080();
    }
    else
    {
        /* No program specified - enter interactive command loop */
        cmdloop();
    }
...
/***********************************************************************
*  CLEANUP - clean up serial I/O and return to ISIS prompt
***********************************************************************/
cleanup()
{
    siouninit();
    ...
    longjmp(cmdloop_env, 1);   /* return to command loop instead of exit() */
}
...
/* Only load program if one was specified on command line */
/* The main() function now handles this logic */

    /* skip past leading spaces on command line to find program name */
    while (f->f_lbuf[f->f_index] == ' ')
	f->f_index++;

    /* Check if there's a program name */
    if (f->f_lbuf[f->f_index] != '\r' && f->f_lbuf[f->f_index] != '\0')
    {
        ...
        if (stat != 0)
        {
	    print("\r\nMissing or non-existent ISIS-II program specified\r\n");
	    cleanup();
        }
    }
}
...
/***********************************************************************
*  READCMD - read a command line from the console
***********************************************************************/

WORD readcmd(buf, maxlen)
BYTE *buf;
WORD maxlen;
{
    WORD count;
    ...
    while (count < maxlen - 1)
    {
        ...
        else if (c >= ' ' && c < 127)  /* printable character */
        {
            buf[count++] = c;
            bdos(2, c);  /* echo character */
        }
    }
    buf[count] = '\0';
    return count;
...
/***********************************************************************
*  PARSECMD - parse command line into program name and arguments
*  Returns pointer to start of program name, and sets argstart to
*  point to the arguments (or 0 if no arguments)
***********************************************************************/

BYTE *parsecmd(cmdline, argstart)
BYTE *cmdline;
BYTE **argstart;
{
    BYTE *progname;
    ...
    {
        *argstart = (BYTE *)0;
        return (BYTE *)0;  /* empty command line */
    }
    ...
    if (*p == '\0')
    {
        *argstart = p;  /* no arguments */
    }
    else
    {
        *p = '\0';  /* null terminate program name */
        p++;
...
/***********************************************************************
*  SETUPCMDTAIL - corrected version
*  Automatically rebuilds the full command line (progname + args)
*  for ISIS-II programs that expect the invoked name to appear first.
***********************************************************************/
...
/***********************************************************************
*  LOADPROG - load an ISIS-II program
*  Returns 0 on success, error code on failure
***********************************************************************/

WORD loadprog(progname)
BYTE *progname;
{
    LBLK pblk;
    WORD stat;
...
/***********************************************************************
*  CLOSEALLFILES - close all open files (except console)
***********************************************************************/
...
/***********************************************************************
*  CMDLOOP - ISIS-II command line loop
***********************************************************************/
...
void cmdloop()
{
    ...
    if (setjmp(cmdloop_env)) {
        /* Returned here after program exit (via cleanup) */
        print("\r\n");
    }
    ...
    while (1)
    {
        ...
        go8080();  /* Run program — if it calls EXIT, cleanup() jumps back here */
        closeallfiles();
        print("\r\n");
    }
}
